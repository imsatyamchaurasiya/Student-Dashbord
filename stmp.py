...
st.success("👉 Left sidebar se 'scorecard' page open kare")
...
Maths=st.number_input("Enter the marks obtained in Maths")
Science=st.number_input("Enter the marks obtained in Science")
English=st.number_input("Enter the marks obtained in English")
Hindi=st.number_input("Enter the marks obtained in Hindi")
SST=st.number_input("Enter the marks obtained in SST")
btn=st.button("Predict Student marks")
if btn:
    model = pickle.load(open("stmp2.pkl", "rb"))
    result = model.predict([[Maths, Science, English, Hindi, SST]])
